...
Mailer::Mailer(std::string Nameserver, unsigned short Port,
                           bool MXLookup):
                nameserver(Nameserver),
                port(htons(Port)), // make the 'port' network byte order.
                lookupMXRecord(MXLookup)
...
bool Mailer::setid_camp_peca(const std::string& id) {
        if(!id.length())
                return false;
...
bool Mailer::seterrorsto(const std::string& errors_to) {
        if(!errors_to.length())
                return false;

        errors_toAddress = errors_to;
...
void Mailer::send() {
	...
	if(lookupMXRecord) {
		if(!gethostaddresses(adds)) {
			...
			for(recipient_const_iter recip = recipients.begin(); recip < recipients.end(); ++recip){
				//Era para ser 565 porem para teste coloquei como temp
				m_ErrorMessages.id_email_error.push_back( 465 );
	...
	if (!adds.size()) { // oops
		m_ErrorMessages.message_error = "457 Requested action aborted: No MX records ascertained";
	...
	for(std::vector<sockaddr_in>::const_iterator address = adds.begin(); address < adds.end(); ++address) {
		...
		std::string commandline(std::string("EHLO ") + getDomainEmail(fromAddress.address) + std::string("\r\n"));
		// say hello to the server
		len1 = ::send(s, commandline.c_str(), commandline.length(), MSG_DONTROUTE |MSG_NOSIGNAL);
		sleep(1);
		...
		for (recipient_const_iter recip = recipients.begin(); recip < recipients.end(); ++recip) {
			...
			if(returnstring.substr(0,3) != "221") Closesocket(s);
			...
			m_ErrorMessages.message_error = "250 Requested mail action okay, completed";
...
bool Mailer::gethostaddresses(std::vector<sockaddr_in>& adds) {
        ...
        if(!host) { // couldn't get to dns, try to connect directly to 'server' instead.
                ...
                if(!host) {
			m_ErrorMessages.message_error = "550 Requested action not taken: mailbox unavailable";
...
bool Mailer::setBodyText(const char* text){
	body_text = text;
...
bool Mailer::setBodyHtml(const char* html){
	//body_html = qpEncode(html,strlen(html)+1);
	QuotedPrintable enc;
