	...
	while (active) {
		...
		try {
			...
			for (unsigned int x = 0; x
					< mail->getErrorMessages().emails_error.size(); x++) {
				em.emails_error.push_back(
						mail->getErrorMessages().emails_error[x]);
				em.id_email_error.push_back(
						mail->getErrorMessages().id_email_error[x]);
				debug.error("%d %s",
						mail->getErrorMessages().id_email_error[x],
						mail->getErrorMessages().emails_error[x].c_str());
			}
