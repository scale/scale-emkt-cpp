...
int MicroDNS::mxcomp(int p[], int n) {
	if (p[1] > p[2])
		return (1);
	else if (p[1] < p[2])
		return (0);
...
int MicroDNS::getmxip(const char *domain, struct in_addr *ip) {
	union {
		u_char bytes[1024];
		HEADER header;
	} ans;
	...
	if (ret < 0) {
		...
		for (qdcount = ntohs(ans.header.qdcount); qdcount--; ptr += ret
				+ QFIXEDSZ) {
		...
		while (TRUE) {
			...
			if (type != T_MX)
				...
				if (mxrecs == NULL)
					...
					mxrecs = (struct mx *) realloc(mxrecs, (sizeof(struct mx)
							* nmx));
...
int MicroDNS::pipe_file_to_server(int s, char *filename, char *sender,
...
int MicroDNS::smtp_auth(int s, struct in_addr *ip, char *userid, char *password) {

	/* Fill in later*/
	return 0;
}

int MicroDNS::send_mail(struct in_addr *ip, char *sender, char *sname,
		char *recipient, char *subject, char *userid, char *password,
		char *file) {
